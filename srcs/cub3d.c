...
int	ft_check_args(int argc, char **argv, int *save_option)
{
	...
	{
		if (ft_strchr(argv[1], ".cub") == XX) //Fonction pour trouver mot dans phrase a déplacer dans parsing
		{
...
int	main(int argc, char **argv)
{
	...
	if (!ft_parsing(argv[1]))
		return (EXIT_FAILURE);    //Affichage des erreurs sur fichier et parsing sera gere dans ft_parsing
